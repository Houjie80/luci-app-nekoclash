...
<body> 
    ...
    <form method= "post" action=""> 
        <label for="subscribeUrl">Subscribe link address:</label> 
        ...
        <div class="button-group"> 
            ...
            <button type="button" class="return-button" onclick="window.location.href='/nekoclash/upload_sb.php';">Return to the previous level</button> 
            <button type="button" class="return-danger" onclick="window.location.href='/nekoclash';">Return to the main menu</button> 
        </div> 
    </form> 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['generateConfig'])) { 
        ...
        if (file_put_contents($configFilePath,$updatedContent) === false) { 
            ...
            $logMessages[] = "Subscription URL generated and downloaded: " . htmlspecialchars($completeSubscribeUrl); 
        } 
        ...
        echo "<button class='copy-button' type='button' onclick='copyToClipboard()'><i class='fas fa-copy'></i> Copy to clipping</button>"; 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['saveContent'])) { 
        if (isset($_POST['configContent'])) { 
            ...
            if (file_put_contents($configFilePath, $editedContent) === false) { 
                ...
                echo "<div class='log-container'>Content has been successfully saved to: " . htmlspecialchars($configFilePath) . "</div>"; 
            } 
        } 
    } 
